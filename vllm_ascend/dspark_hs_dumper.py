...
class DsparkHSDumper:
    ...
    def capture_from_state(self, state) -> None:
        """MRV2 entry point: unpack an ``ExecuteModelState`` and delegate to ``capture``.

        Kept as a thin adapter so the validated accumulate/flush core below stays
        byte-for-byte the code that produced the existing hs_*.safetensors corpus.

        Three things moved between model-runner v1 and MRV2:
          * ``scheduler_output`` is no longer needed -- per-request scheduled token counts
            are on the batch as ``num_scheduled_tokens`` (ndarray, batch-index keyed)
            instead of a dict keyed by req_id.
          * ``input_ids`` lives on the batch rather than on the runner (``self.input_ids.gpu``).
          * ★ aux changed SHAPE CONVENTION: v1's ``get_mtp_target_hidden_states()`` returned one
            flattened [T, L*H] scratch buffer; MRV2 hands us ``list[Tensor]``, L tensors of [T, H].
            Stacking on dim=1 reproduces the SAME [seq, L, H] the flush path expects, so the
            written file format is unchanged and the trainer's ArrowDataset reads it as before.
        """
        if not self.active or state is None:
            return
        aux_list = state.aux_hidden_states
        if not aux_list or state.hidden_states is None:
            return
        batch = state.input_batch
        self.capture(
            num_scheduled_tokens=batch.num_scheduled_tokens,
            req_ids=batch.req_ids,
            prompt_lens=batch.prefill_len_np,
            hidden_states=state.hidden_states,
            aux_buffer=torch.stack(list(aux_list), dim=1),   # [T, L, H]
    ...
    def capture(
        ...
        *,
        ...
        ``aux_buffer``     : [>=num_tokens, L_aux, H]   aux target layers, already stacked
        ``input_ids``      : [>=num_tokens]             token ids for this forward
        ``num_scheduled_tokens`` / ``prompt_lens`` : [num_reqs], indexed by BATCH position
        ``req_ids``        : [num_reqs], same order
    ...
    def _flush(self, req_id) -> None:
        ...
        if ent is None:
            return
        aux = torch.cat(ent["aux"], dim=0)    # [seq, L_aux, H]  (aux target layers, stacked)
