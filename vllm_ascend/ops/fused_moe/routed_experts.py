...
class AscendUnquantizedFusedMoEMethod(UnquantizedFusedMoEMethod):
    ...
    def process_weights_after_loading(self, layer):
        ...
        if enable_fused_mc2:
            ...
            # Free the stacked weights once the per-expert list exists. The mega-MoE branch
            # built the list and kept the original [E, ...] tensors alive, so every routed
            # expert weight sat in device memory TWICE -- the list entries are clones of the
            # same data, not views. On DSV4-Flash bf16 that is the difference between fitting
            # and OOM. Pure memory: the clones are bit-identical, so no output changes.
